...
function isValidItem(item: HistoryItem): boolean {
  ...
  if (SYSTEM_HINTS.some((hint) => text.includes(hint))) return false;
  return true;
}
...
interface TranslationHistoryProps {
  ...
  exportSessionTxt?: (sessionId: string, mode?: ExportMode) => boolean | undefined;
}
...
const exportModes: Array<{ value: ExportMode; label: string; desc: string }> = [
  { value: 'bilingual', label: '双语对照', desc: '一句原文 + 一句译文' },
  { value: 'original_first', label: '原文优先', desc: '全部原文在前，全部译文在后' },
  { value: 'translation_only', label: '仅译文', desc: '只导出中文翻译' },
];

/** 导出弹窗 */
function ExportModal({ session, onClose, exportSessionTxt }: {
  session: Session;
  onClose: () => void;
  exportSessionTxt?: (sessionId: string, mode?: ExportMode) => boolean | undefined;
}) {
  const [format, setFormat] = useState<'txt' | 'docx'>('txt');
  const [mode, setMode] = useState<ExportMode>('bilingual');
  const [exporting, setExporting] = useState(false);
  const itemCount = session.items.filter(isValidItem).length;

  const handleExport = async () => {
    setExporting(true);
    try {
      if (format === 'txt' && exportSessionTxt) {
        exportSessionTxt(session.id, mode);
      } else if (format === 'docx') {
        const items = session.items.filter(isValidItem).map((it) => ({
          original: it.original,
          translated: it.translated,
        }));
        await generateDocx(items, mode);
      }
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="bg-slate-800 rounded-xl p-6 w-80 border border-slate-600 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <h3 className="text-white font-bold mb-1">导出会话</h3>
        <p className="text-xs text-slate-500 mb-4">{itemCount} 条有效翻译</p>

        <div className="space-y-4">
          <div>
            <label className="text-slate-400 text-xs font-medium">文件格式</label>
            <div className="flex gap-2 mt-1.5">
              <button onClick={() => setFormat('txt')} className={`flex-1 px-3 py-2 rounded-lg text-sm transition-all ${format === 'txt' ? 'bg-cyan-600 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}>TXT</button>
              <button onClick={() => setFormat('docx')} className={`flex-1 px-3 py-2 rounded-lg text-sm transition-all ${format === 'docx' ? 'bg-cyan-600 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}>Word</button>
            </div>
          </div>

          <div>
            <label className="text-slate-400 text-xs font-medium">排版模式</label>
            <div className="flex flex-col gap-1.5 mt-1.5">
              {exportModes.map((m) => (
                <button
                  key={m.value}
                  onClick={() => setMode(m.value)}
                  className={`px-3 py-2 rounded-lg text-left text-sm transition-all ${mode === m.value ? 'bg-cyan-600/20 text-cyan-300 border border-cyan-500/40' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}
                >
        ...
        <div className="flex justify-end gap-2 mt-6">
          ...
          <button onClick={handleExport} disabled={exporting} className="px-4 py-2 text-sm bg-cyan-600 text-white rounded-lg hover:bg-cyan-500 transition-all disabled:opacity-50">{exporting ? '导出中...' : '导出'}</button>
        </div>
      </div>
    </div>
  );
...
export function TranslationHistory({ sessions, onToggleSession, onDeleteSession, onClear, generateTitle, exportSessionTxt }: TranslationHistoryProps) {
                ...
                >
                  ...
                  <div className="px-3 py-2 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1 ml-2">
                      {/* 导出按钮 */}
                      {session.items.filter(isValidItem).length > 0 && (
                        <button
                          onClick={(e) => { e.stopPropagation(); setExportingSession(session); }}
